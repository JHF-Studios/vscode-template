...
export async function main(ns:NS) {
    ...
    while(true){
        ...
        slist.forEach(s => {
            if (ns.getServerMaxMoney(s) > ns.getServerMaxMoney(target) && ns.getServerRequiredHackingLevel(s) <= ns.getHackingLevel() && ns.getServer(s).hasAdminRights) target = s;
        })
        ...
        slist = slist.filter(s => {return ns.getServer(s).hasAdminRights && ns.getServerMaxRam(s) !== 0});
        ...
        while(true){
            ...
            for(const server of slist){
                ...
                const threads = Math.floor((serverMaxRam - usedRam) / Math.ceil(weakenCost));
                switch(action){
                    case 'weakening':
